...
The final ZIP was extracted outside the checkout and its app installed locally. The DMG was mounted read-only, verified and detached. Both passed architecture, deployment-target, localization, signature and system-library dependency checks. Original audio and manifests remained unchanged. These checks were performed on the unpublished 0.4.1 build, before the 0.4.2 preview release.
